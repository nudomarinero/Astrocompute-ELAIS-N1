...
output_name_template = "L{}_SBgr{:03d}-10_uv.MS.pre-cal.ms.tar"
...
def get_name(input_name):
    ...
    freq = int(freq_str)
    return output_name_template.format(L, freq2band[freq])
...
def rename_directory(directory, verbose=True):
    """
    Rename all the relevant bands in a directory
    """
    mss = glob("{}/L*_SAP???_SB???_uv.MS_*_???MHz.pre-cal.ms.tar".format(directory))
    #print(mss)
    for ms in mss:
