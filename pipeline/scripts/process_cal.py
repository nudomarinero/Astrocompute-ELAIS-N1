...
if os.path.exists("/home/ubuntu/logging.conf"):
    ...
    # Log to file
    file_name = "/home/ubuntu/pipeline_cal.log"
...
def launch(step):
    ...
    try:
        ...
        message = ("Exception in LOFAR AWS cal pipeline\n"+
            "Dataset: {}\n".format(dataset)+
            "Data id:    {}\n".format(band)+
            "Step:    {}\n".format(fname)+
            "Error:\n"+
        ...
        notify(message, 
               subject="Exception LOFAR AWS cal; step: {}; band: {}".format(fname, band))
        raise
...
def run_pipeline():
    ...
    for line in pipeline("-c",
                          ...
                          "/home/ubuntu/astrocompute/pipeline/generic_pipeline/pre_facet_cal_rawdata.parset",
                         _iter=True):
        print(line)
...
def upload_data():
    ...
    """
    upload = sh.Command("/home/ubuntu/upload_data_cal.sh")
